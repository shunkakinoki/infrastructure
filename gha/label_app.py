...
class GitHubApp(GitHub):
    ...
    def __init__(self, pem_path, app_id):
        ...
        if not self.path.is_file():
            raise ValueError(
                f"""argument: `pem_path` must be a valid filename.
                {pem_path} was not found."""
            )
    ...
    def generate_installation_curl(self, endpoint):
        ...
        print(
            f"""curl -i -H "Authorization: token {iat}" -H
            "Accept: application/vnd.github.machine-man-preview+json"
...
if __name__ == "__main__":
    ...
    if trigger_label and not (pem and app_id):
        raise EnvironmentError(
            """If you supply a value for INDICATOR_LABEL you must also provide APP_PEM
            and APP_ID to authenticate as a GitHub App."""
        )

    assert (
        github_token
    ), "Error: system environment variable GITHUB_TOKEN must be provided."
    assert trigger_phrase, "Error: must supply input TRIGGER_PHRASE"
    assert (
        payload_fname or test_payload_fname
    ), """Error: System environment variable GITHUB_EVENT_PATH
    or TEST_EVENT_PATH not found"""

    fname = payload_fname if not test_payload_fname else test_payload_fname
    owner, repo = os.getenv("GITHUB_REPOSITORY").split("/")  # type: ignore

    with open(fname, "r") as f:  # type: ignore
        payload = json.load(f)
    ...
    assert (
        "issue" in payload and "comment" in payload
    ), "Error: this action is designed to operate on the event issue_comment only."
    ...
    if "pull_request" in issue_data and trigger_phrase in comment_data["body"]:
        ...
        response = get_pr_metadata(
            owner=owner, repo=repo, pr_number=issue_number, token=github_token
        )
        assert (
            response
        ), f"Error: unable to retrieve PR metadata: {response.status_code}"
        head_branch = response.json()["head"]["ref"]
        head_sha = response.json()["head"]["sha"]

        if trigger_label and pem:
            issue_handle = get_issue_handle(
                owner=owner,
                repo=repo,
                pem=pem,
                app_id=app_id,
            ...
            assert (
                result and trigger_label in labels
            ), "issue annotation on PR not successfull."
            print(
                f"""Successfully added label {trigger_label} to {issue_handle.state}
                PR: {issue_handle.html_url}"""
            )

        # emit output variablesOne w
        trailing_text = comment_data["body"].split(trigger_phrase)[-1]
        trailing_line = (
            trailing_text.splitlines()[0].strip() if trailing_text.splitlines() else ""
        )
